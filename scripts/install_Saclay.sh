...
if [ "$#" -ne 3 ]; then
 echo "invalid number of arguments. You must specify the names of the virtual environment to build, the folder with cross-section library (the folder that contains cross_sections.xml) and the folder with MEDCoupling files."
 exit
fi
...
pip install venv-modulefile==0.0.24
...
venvmod-cmd-setenv ${install_path} OPENMC_CROSS_SECTIONS ${cross_section_path}/cross_sections.xml
...
venvmod-add-appli ${install_path} MEDCOUPLING
